...
涉及**量化交易**相关内容（策略、指标、回测、实盘对接等）时，正文、标题、摘要、标签、代码注释与链接锚文本 **不得** 出现具体交易所或交易平台名称（如中心化加密交易所、券商、期货平台等的品牌名）。

| 要求       | 说明                                                                                                                      |
| ---------- | ------------------------------------------------------------------------------------------------------------------------- |
| 统一写法   | 用「交易所」「交易平台」代替具体品牌名；需要区分环境时可写「测试网」「主网」，不写平台专有名称                            |
| 允许保留   | 自己使用/开发的开源项目名称本身（如 QuantDinger），因为它是文章讨论的主体，不是被对接的交易所                             |
| 交易对符号 | 代码示例里的交易对（如 `BTCUSDT`）、通用协议字段名可以保留，不涉及平台品牌                                                |
| 标签       | `tags` 中不要用交易所名称作标签（如具体交易所英文名），改用通用技术标签（如 `quant`、`trading-strategy`、`exchange-api`） |

**示例**：

- ❌ "在 UI 上跑通了某交易所的挂单和平仓"（写出具体交易所名称）
- ✅ "在 UI 上跑通了交易所的挂单和平仓"
...
1. 新建或编辑 `content/post/**/*.md` 时，**先读本文**（语气平实、VPN 命名、交易所命名、代码块注释等）。
2. 涉及 VPN/代理 homelab 场景：成稿后全文检索，WireGuard / OpenVPN 保留，其余 VPN 软件名须替换。
3. 涉及量化交易场景：成稿后全文检索交易所/交易平台品牌名并替换为通用词，标签同步检查。
4. 成稿后检查 fenced code block：注释是否为英文；中文解释是否已移到代码块外。
5. SDD 文章：单篇 Spec §禁止包含 可补充本篇特例，**不得**默认放宽 VPN、交易所命名或代码注释约束。
...
| 2026-07-08 | 新增：量化交易文档不写交易所/交易平台具体名称              |
