...
JPDA 三层：**JDI**（调试器 Java API）→ **JDWP**（线协议）→ **JVMTI**（JVM native）。JVMTI 专文见 [JVMTI](./jvmti.md)。开发期改已加载类（HotSwap）走本体系，与 [BTrace](./btrace.md) attach 观测不是同一路；热替换总览见 [开发期热替换](../../cs/dcevm-hotswapagent.md)。
