...
JDK 1.4 先后引入了 Parallel Scavenge（1.4.0）和 CMS、ParNew（1.4.2，均为实验特性），到 1.4 系列结束时，可用的新生代 + 老年代组合如下（详见 [Java GC](./gc.md)）：

| 新生代收集器 | 老年代收集器 | 启用参数 | 说明 |
| ---- | ---- | ---- | ---- |
| Serial | Serial Old | `-XX:+UseSerialGC` | 自 JDK 1.3.1 及以前就是唯一选择，1.4 中仍可用 |
| Parallel Scavenge | Serial Old | `-XX:+UseParallelGC` | 1.4.0 引入；此时 Parallel Old 还未出现（JDK 1.6 才有），且 Parallel Scavenge 无法与 CMS 配合，老年代只能选 Serial Old |
| ParNew | CMS | `-XX:+UseParNewGC` 搭配 `-XX:+UseConcMarkSweepGC` | 1.4.2 随 CMS 一起以实验特性引入；ParNew 是 Serial 的多线程版本，存在的主要目的就是配合 CMS 收集新生代 |
| ParNew | Serial Old（后备） | （Concurrent Mode Failure 时自动切换） | 非正常配置，是 CMS 并发失败时的后备预案，停顿时间会明显变长 |

ParNew 是新生代收集器，本身不属于 CMS 的一部分；但因为 Parallel Scavenge 无法与 CMS 搭配，ParNew 事实上是当时唯一能配合 CMS 工作的并行新生代收集器，所以两者常被一起提及。
...
| 2026-07-14 | 新增「GC 组合速查」章节，补充可用新生代/老年代收集器组合表格 | 说明 ParNew 作为新生代收集器的配套老年代收集器 |
| 2026-07-15 | url 改为 `jdk-1-4` | URL 含 `.` 时 Cloudflare 误判 MIME，页面无法打开 |
