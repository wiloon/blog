...
**说明：** `@Autowired` 字段注入在 Spring 6 / Boot 3 中 **并未标记为 `@Deprecated`**，编译和运行都正常。业界说的「要淘汰」指的是 **工程实践上不推荐**，属于 Spring 生态（Framework + Boot + IDE 检查）的共识，而不是 Boot 单独废除了该 API。新 Bean 应优先构造器注入；详见 [Spring Boot 依赖注入方式](./spring/spring-boot.md#依赖注入方式)。
...
- [Spring Boot 依赖注入方式](./spring/spring-boot.md#依赖注入方式)（构造器注入、字段注入为何不推荐、不可变 `final` 依赖）
