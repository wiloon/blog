...
```text
app.jar
├── META-INF/MANIFEST.MF     # 启动元数据，关键项见下表
...
JVM 执行 `java -jar app.jar` 时**不会先把 jar 解压到磁盘**，而是用 ZIP 随机访问直接读取。

### JVM 如何定位 MANIFEST.MF

jar 本质是 ZIP，ZIP 把**中央目录**（Central Directory）放在文件**末尾**，所以 JVM 不必从头读：

```text
① seek 到文件末尾 → 读 End of Central Directory（EOCD，定长）
② 从 EOCD 拿到中央目录偏移 → seek 过去
③ 在目录中找到 META-INF/MANIFEST.MF 条目 → 得到其字节偏移
④ seek 到该偏移 → Deflate 解压 → 读出 MANIFEST.MF
```

只需 3 次 seek，不触碰其它文件。ZIP「每个文件独立压缩、可随机访问」的特性见 [Compress and Extract](../../../linux/compress.md) §归档格式与随机访问。

### 类按需懒加载

确定 `Main-Class`（`JarLauncher`）后整体时序：

```text
java -jar app.jar
  ↓ OS fork 进程，加载 libjvm.so（JVM 主体）
  ↓ JVM 初始化堆 / GC / JIT
  ↓ 3 次 seek 读 MANIFEST.MF（不解压整包）
  ↓ 加载顶层 JarLauncher（标准 ClassLoader 可直接读）
  ↓ JarLauncher 用 NestedFileSystemProvider 挂载 BOOT-INF/lib/ 内嵌 jar
  ↓ 反射调用 Start-Class.main() → SpringApplication.run()
      → 创建 ApplicationContext → refresh() → 启动内嵌服务器
```

`.class` 文件**按需懒加载**——只有某个类首次被用到时才从 jar 读出并解压，磁盘上的 jar 全程保持压缩。`libjvm.so` 与 `java` 启动器的关系见 [HotSpot](../hotspot.md) §libjvm.so 与 JVM 主体。

`SpringApplication.run()` 内部的 Spring 装配分三段（详见 [Spring IoC §refresh()](./spring-ioc.md#启动时-refresh-概览)）：扫描登记 Bean 定义（用 ASM 读字节码元数据，**不加载类**）→ 实例化单例 → 注入依赖。

## META-INF 里还有什么

`META-INF` 把 jar 从「哑压缩包」变成**自描述模块**。可执行 fat jar 的 `META-INF` 常见内容：

```text
META-INF/
├── MANIFEST.MF                       # 启动元数据，见上文
├── maven/<group>/<artifact>/
│   ├── pom.xml                       # 打包时拷入的原始 pom
│   └── pom.properties                # groupId/artifactId/version，便于快速查询
└── services/
    └── java.nio.file.spi.FileSystemProvider   # Java SPI 注册
```

- `maven/...`：Maven 打包时嵌入，`mvn dependency:tree` 等工具据此识别 classpath 上的构件。
- `services/`：[Java SPI](../java-spi-serviceloader.md) 注册目录。
...
标准 JVM 无法表示「jar 里再嵌 jar」的路径。`NestedFileSystemProvider` 向 JVM 注册了一个新的 `nested:` URI scheme，使下面这种路径可用：

```text
nested:/path/to/app.jar/!BOOT-INF/lib/spring-core-6.x.jar
```

这让 `JarLauncher` 能把 `BOOT-INF/lib/` 下每个嵌套 jar 当作独立文件系统打开并加载类——**正是 fat jar 无需解压即可运行的技术基础**。MANIFEST.MF 各字段含义见 [MANIFEST.MF](../../../other/manifest-mf.md)。
...
本站笔记：[Maven 可执行 jar](../../../cs/maven-executable-jar.md)、[Gradle application 插件](../../../cs/gradle-executable-jar.md)、[maven-shade-plugin](../../../cs/maven-shade-plugin.md)。

Maven Shade 那种「把所有依赖的 class 解压平铺进一层」的做法有几个明显问题，正是 Spring Boot 保留嵌套 jar 的原因：

| 问题 | 说明 |
| ---- | ---- |
| **文件冲突** | 多个 jar 含同名文件（`LICENSE`、`META-INF/spring.factories`），合并后只剩一份 |
| **SPI 失效** | `META-INF/services/` 下同名文件相互覆盖，丢失实现 |
| **签名失效** | 解压已签名 jar 会破坏其签名 |
| **可追溯性差** | 无法判断某个 class 来自哪个依赖的哪个版本 |

Spring Boot 把依赖 jar 原样放在 `BOOT-INF/lib/`，靠 `NestedFileSystemProvider` 加自定义 ClassLoader 读取，规避了上述全部问题；这也是为了加快打包、并支持后续分层 JAR（`layers.idx`）与 Buildpack 切层——分层与容器化见 [Spring Boot Container Packaging](./spring-boot-container-packaging.md)。
...
| 时间 | 修改内容 | 原因 |
| ---- | -------- | ---- |
| 2026-06-26 | 精简布局注释中 `Main-Class` / `Start-Class` 说明 | 与下方 MANIFEST.MF 表格重复，只在表格处保留一处 |
| 2026-06-27 | 新增「启动全流程」（ZIP 随机访问、类懒加载）、「META-INF 里还有什么」（maven/、SPI、NestedFileSystemProvider）；补充扁平解压的问题表 | 合并 comments-tree 启动打包文档相关章节 |
