...
lastmod: 2026-07-14T18:10:44+08:00
url: jdk-7
categories:
  - language
tags:
...
- G1 垃圾收集器（6u14 起实验性提供，7u4 起转为正式支持）
...
G1（Garbage-First）最早在 **JDK 6u14** 以实验性选项引入；**JDK 7u4 起转为正式支持**（`-XX:+UseG1GC`），面向大堆、可预测停顿，但仍需显式开启，不是默认项；JDK 9 起才成为默认 GC。详见 [InfoQ：JDK 7 G1](http://www.infoq.com/cn/articles/jdk7-garbage-first-collector)。
...
| Parallel（默认） | `-XX:+UseParallelGC` | Parallel Scavenge + Parallel Old，吞吐量优先 |
| CMS | `-XX:+UseConcMarkSweepGC` | ParNew + CMS，低停顿；此时还没有被标记废弃（标记废弃是 JDK 9 的 JEP 291） |
| G1 | `-XX:+UseG1GC` | JDK 6u14 实验性引入，JDK 7u4 起转为正式支持；仍需显式开启，不是默认项 |

ZGC、Shenandoah、Epsilon 都是 JDK 11/12 才出现的；针对过时 GC 组合的 JEP 173（标记废弃）、JEP 214（移除）也是 JDK 8/9 之后的事，JDK 7 里还不涉及。完整时间线见 [Java GC](./gc.md#收集器发展时间线)。
...
| 2026-07-14 | 修正 G1 描述（实验性引入是 6u14，7u4 起才是正式支持，原文误写为 7u4 实验性提供）；新增「JDK 7 可用收集器速查」表格 | 原描述与事实不符；用户要求补充可用 GC 列表 |
