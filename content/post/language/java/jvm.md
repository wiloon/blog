...
安装 `java` 命令得到的是 **JDK 发行版**（见 [openjdk](./openjdk.md)），其中默认捆绑的虚拟机实现几乎都是 **HotSpot**。本文介绍 Java 领域常见实现、差异与选型；**HotSpot 的参数与工具**见 [hotspot](./hotspot.md)。
...
| **GraalVM** | GraalVM CE/EE | 可嵌 HotSpot 作 JIT 运行时，或通过 **Native Image** AOT 为本地可执行文件 | 极致冷启动（serverless、CLI）；Native Image 对反射、动态加载有限制 |
...
3. **冷启动极敏感**（函数计算、短生命周期进程），评估 **GraalVM Native Image**；需接受构建期分析、反射配置、动态类加载受限等成本。
...
| OpenJDK 发行版与安装 | [openjdk](./openjdk.md) |
