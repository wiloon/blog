...
[BTrace](./btrace.md)、Arthas、Spring LTW、部分 AOP 框架都会在 **不改变业务源码** 的前提下改写字节码。底层常依赖 **ASM**（或其它字节码库）配合 JVM 的 **`java.lang.instrument`** API。
...
1. **[Attach API](./attach-api.md)** `loadAgent(btrace-agent.jar)`：在 **同一业务进程** 内加载 agent，调用 **`agentmain`**（不走业务 `main`，也不走 `AppClassLoader` 去 `java -jar` agent）。
...
[BTrace](./btrace.md) 则在 **脚本编译/校验** 阶段限制：不能 `new`、不能循环、不能抛异常等，并把 API 收敛到 `BTraceUtils`，目的是 **可观测、低侵入**，而不是 JVM 装了一个「agent 沙箱」。
...
「不经过 AppClassLoader」指的是 **加载 agent JAR 里类的机制** 与加载 `com.yourapp.*` 的路径不同，避免 agent 与业务 classpath 搅在一起；不是另起一个 JVM。子加载器能否看到父加载器上的类、兄弟加载器是否互不可见，见 [java classloader](./classloader.md)#不同-classloader谁能看到谁。
