...
lastmod: 2026-06-19T22:16:52+08:00
url: jvm-compiler
categories:
  - language
aliases:
...
OpenJDK 默认的 [HotSpot](../language/java/hotspot.md) 虚拟机通过 **解释器 + 即时编译（JIT）** 执行字节码：冷代码先解释运行，热点方法再编译为本地机器码。本文归纳 C1/C2 分工、分层编译与热点探测；JDK 21 之后的 AOT 预热见 [JDK 25 变更汇总](../language/jdk25-changes-from-jdk21.md)。
...
详见 [JDK 21 升级到 JDK 25 的变更汇总](../language/jdk25-changes-from-jdk21.md)；JDK 26 对 AOT 对象缓存的扩展见 [JDK 26 相对 JDK 25 的变化](../language/java/jdk-26.md)。相关 JVM 参数见 [HotSpot JVM 启动参数](../language/java/hotspot-options.md)。
