...
lastmod: 2026-07-09T16:12:46+08:00
url: java
categories:
  - Java
tags:
...
详见 [HashMap 与 Hashtable 的区别](./hashmap-vs-hashtable.md)。
...
`interface` 声明一组方法契约，类通过 `implements` 实现；一个类可实现多个接口，从而弥补 Java 单继承的限制。接口中的实例方法默认 `public`；字段（若定义）默认 `public static final`。Java 8 起接口还可含 `default` 与 `static` 方法。

语法对比、is-a / like-a 设计取舍及 Door/Alarm 示例见 [Java 抽象类与接口](./abstract-class-and-interface.md)。Lambda 所用的函数式接口见 [Java 8 函数式接口](../../other/functional-interface.md)。
...
| 时间 | 修改内容 | 原因 |
| ---- | -------- | ---- |
| 2026-06-20 | 移出 JavaEE、Servlet 段落；精简 interface 内容为链到 `abstract-class-and-interface.md` 与 `functional-interface.md` | 与 Java 语言基础主题对齐；interface 专题独立成文 |
| 2026-06-20 | 移出内容归档：JSP include → `web/jsp-include.md`；forward/redirect → `web/jsp-forward-redirect.md`；EJB vs JavaBean → `java-bean.md`；Servlet/CGI → `other/servlet-lifecycle.md` | 恢复被删段落至合适专题文 |
| 2026-06-21 | 维护记录中 Servlet 专题文路径改为 `servlet-lifecycle.md` | 专题文文件名英文化 |
| 2026-07-09 | HashMap 和 Hashtable 段落改为链接到 `hashmap-vs-hashtable.md`；重复的 HashMap/Hashtable 对比小节合并 | 去除与专题文重复的内容 |
