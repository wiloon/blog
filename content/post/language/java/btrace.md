...
```text
Client（Attach API + 脚本编译）+ Agent（ASM + Instrumentation）+ Socket
```
...
2. **Attach API 加载 Agent**
   ...
   `loadAgent` 把 **btrace-agent.jar** 载入业务 JVM，调用 `agentmain`，获得 `Instrumentation`。**不是** 载入脚本 jar。
