...
概念与相关文章的关系见 [Java 领域知识关系图](./java-knowledge-map.md)。
...
| `java -jar app.jar` | [Attach API](./attach-api.md) `loadAgent` 或启动时 `-javaagent`（`premain`） |
...
命名空间与「子可见父、兄弟互不可见」见 [java classloader](./classloader.md) §不同 ClassLoader。
...
| 与 Spring DevTools | 无关 | [DevTools](./spring-boot-devtools.md) 是上下文 Restart，不是 JVM HotSwap |
...
2. **Attach API 加载 Agent**
   ...
   客户端使用 [Attach API](./attach-api.md)（`com.sun.tools.attach.VirtualMachine`）：
...
| 开发热替换（对比用） | [开发期热替换](../../cs/dcevm-hotswapagent.md)、[Spring Boot DevTools](./spring-boot-devtools.md) |
| jcmd、工具选型 | [jcmd](./jcmd.md)、[生产环境诊断工具选型](./java-production-diagnostics-tooling.md) |

## 参考

- [BTrace 项目](https://github.com/btraceio/btrace)
- [生产环境诊断工具选型](./java-production-diagnostics-tooling.md)（其它 JDK 版本下的工具对照）
...
- JFR 自 JDK 11 起免费，许多场景可用 `jcmd <pid> JFR.start` 做 GC、锁、分配等分析，见 [Java Flight Recorder](./java-flight-recorder-jfr.md)。
...
详见 [Java 生产环境诊断工具选型](./java-production-diagnostics-tooling.md)。
