...
```bash
# 对文件的修改还没有提交(没有执行过 git add, git commit), 撤消本地的修改, 已经 add/commit 的不适用

# 推荐：新版本使用 git restore（Git 2.23+）
git restore .              # 撤销所有未暂存的修改
git restore <file>         # 撤销指定文件未暂存的修改

# 旧版本写法（仍然有效）
git checkout . # 本地所有修改的。没有的提交的，都返回到原来的状态
git checkout -- <file>     # 撤销指定文件未暂存的修改
```
