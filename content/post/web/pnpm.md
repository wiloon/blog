...
npm 从 5.2 版开始，增加了 `npx` 命令。`npx` 想要解决的主要问题，就是调用项目内部安装的模块。

#### npx 的核心功能

1. **调用项目内部模块** — 无需全局安装，直接运行 `node_modules/.bin/` 下的可执行文件
2. **免安装执行** — 若本地和全局都没有安装，会临时下载、执行后删除，不污染全局环境
3. **执行指定版本** — 可以直接运行指定版本的包，例如 `npx node@14 -e "console.log(process.version)"`

```bash
# 调用项目内部安装的 mocha，而不用写 ./node_modules/.bin/mocha
npx mocha
...
# 使用 --no-install 禁止自动下载（本地没有就报错）
npx --no-install mocha

# 使用 --ignore-existing 强制临时下载，即使本地已安装
npx --ignore-existing create-react-app my-app
```

#### 示例命令解析：`npx skills add`
...
- **`npx skills`** — 临时下载并执行 npm 上的 `skills` 包（一个用于管理 GitHub Copilot 技能/指令文件的 CLI 工具）
- **`add`** — `skills` CLI 的子命令，表示添加技能
- **`https://github.com/vercel-labs/skills`** — 技能库的来源，从指定的 GitHub 仓库拉取技能
- **`--skill find-skills`** — 指定要添加的技能名称为 `find-skills`

执行效果：从 `vercel-labs/skills` 仓库中下载名为 `find-skills` 的技能文件，安装到当前项目，供 GitHub Copilot Agent 使用。

> **注意：** `npx` 对应 pnpm 的命令是 `pnpm dlx`，例如 `pnpm dlx create-react-app my-app`。
