...
lastmod: 2026-05-22T20:52:27+08:00
...
paru 不在 Arch 官方仓库，只能从 AUR 安装。若系统里已有 yay，可直接：

```bash
yay -S paru-bin
```

`paru-bin` 使用预编译二进制，无需本地编译 paru 本身，比 `paru` 包更快。也可装源码版：`yay -S paru`。

首次安装、尚未有任何 AUR helper 时，需手动从 AUR 构建（与 yay 一样，不能通过 `pacman -S paru` 直接安装）：
...
若 `makepkg -si` 因 sudo 权限失败，可分两步：`makepkg` 构建后，再 `sudo pacman -U paru-*.pkg.tar.zst`。
...
paru 二进制是按旧版 pacman 的 libalpm 编译的，系统升级 pacman 后未同步重装 paru 时会出现此错误。解决：按当前 pacman 重新安装 paru（`yay -S paru-bin` 或 `git clone` + `makepkg -si`）。若只能用 pacman 更新系统，升级后务必重装 paru。
