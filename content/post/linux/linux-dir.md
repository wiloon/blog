...
/opt 目录的主要用途是 存放第三方应用软件包（optional software packages），也就是那些不是由系统默认的包管理器（如 apt, yum, pacman 等）安装的程序。

/opt 的设计目的：
“Optional” 的缩写：/opt = optional，表示这些软件是“可选”的，不属于系统核心组件。

保持系统整洁：与系统自带的程序（一般安装在 /usr/bin, /usr/lib 等）分开，避免混淆。

便于管理与卸载：每个软件通常会在 /opt 下有一个独立的子目录，比如 /opt/google, /opt/kong，便于手动安装与清理。

表示的是可选择的意思，有些软件包也会被安装在这里，也就是自定义软件包，比如在 Fedora Core 5.0 中，OpenOffice 就是安装在这里。有些我们自己编译的软件包，就可以安装在这个目录中；
...
操作系统运行时，进程 (正在运行中的程序) 信息及内核信息 (比如cpu、硬盘分区、内存信息等) 存放在这里。/proc 目录伪装的文件系统 proc 的挂载目录，proc 并不是真正的文件系统，它的定义可以参见 /etc/fstab

/proc 目录下的文件都是虚拟的，实际上它们并不占用磁盘空间，而是内核动态生成的。这个目录下的文件可以用来查看系统的状态和进程信息。

每个正在运行的进程在 /proc 目录下都有一个以其进程号（PID）命名的目录。

这个目录包含了该进程的大量运行时信息，包括但不限于：

文件或子目录	说明
/proc/737543/exe	指向该进程的可执行文件（符号链接）
/proc/737543/cmdline	启动该进程的命令行参数
/proc/737543/environ	环境变量
/proc/737543/status	简要的进程状态信息
/proc/737543/fd/	打开的文件描述符（文件、socket 等）
/proc/737543/maps	内存映射
/proc/737543/cwd	当前工作目录（符号链接）
