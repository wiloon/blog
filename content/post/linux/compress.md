---
title: Compress and Extract 压缩与解压
author: "-"
date: 2011-11-04T05:13:18+00:00
...
```bash
unzip -l myfile.zip
unzip -l /path/to/myfile.zip | head
```

`-l`（list）只列出压缩包内文件清单，不解压。输出列依次为原始大小、日期、时间、文件名。想先看包里有什么、或确认路径对不对时用这个。

更详细的列表（含压缩方式、压缩前后大小、压缩率等）用 `-v`：

```bash
unzip -v myfile.zip
```

非标准 ZIP（例如 Java 的 `.jmod` 文件带 4 字节 `JM` 头）有时也能列出，但开头可能提示 extra bytes；一般查看内容够用。

### zip 读取到 stdout（不落盘）

```bash
# View a text file inside the archive
unzip -p myfile.zip path/to/foo.txt

# Pipe to other commands
unzip -p myfile.zip config.json | jq .
unzip -p myfile.zip README.md | less
grep -i error <(unzip -p logs.zip app.log)
```

`-p`（pipe）把指定条目的**解压后内容**写到 stdout，不在当前目录落盘，也不打印解压进度等提示。适合快速查看文本、或把包内文件交给管道下游（`grep`、`jq`、`less` 等）。

只读单个文件时，在压缩包名后面跟路径即可；路径须与 `unzip -l` 列出的名称一致（含目录层级）。多个文件会按顺序拼接输出到 stdout，中间没有分隔符，一般只对一个条目用 `-p`。

与 `-l` 的区别：`-l` 只看清单；`-p` 是真正解压数据，只是写到管道而非磁盘。与 `unzip foo.zip path/to/foo.txt` 的区别：后者会在当前目录（或 `-d` 指定目录）写出文件；`-p` 不写文件。
...
| 格式 | 压缩方式 | 随机访问 | 压缩率 |
| ---- | -------- | -------- | ------ |
| **ZIP** | 每个文件**独立**压缩 | ✅ seek 到偏移即可 | 一般 |
| **tar.gz** | 所有文件先合并成一个流再压缩 | ❌ 必须从头解压 | 较好 |
| **7z** | 默认 **Solid**：所有文件在一个流里 | ❌ Solid 模式下不支持 | 高（比 ZIP 小 30–50%） |
| **RAR** | 默认 Solid，可关闭 | ❌ Solid 模式下不支持 | 高 |

**Solid 压缩**：把文件拼接后再压缩，压缩器能跨文件发现重复模式（如大量相似的 `.class`），压缩率更高；但要解出第 100 个文件，得先解前 99 个。

**为什么 JAR 必须用 ZIP**：JVM 一次按需懒加载一个 `.class`，Solid 压缩会要求为每次类加载解压整个归档，实践上不可用。详见 [Spring Boot Executable JAR](../language/java/spring/spring-boot-executable-jar.md) §启动全流程。

### ZIP 的内部结构

ZIP 把**中央目录**（Central Directory）放在文件末尾，每个文件独立压缩、独立登记：

```text
┌─────────────────────────────────┐  ← 文件开头
│  Local Header + 压缩数据         │  entry 1
├─────────────────────────────────┤
│  Local Header + 压缩数据         │  entry 2
├─────────────────────────────────┤
│  ...                            │
├─────────────────────────────────┤
│  Central Directory              │  索引：每个 entry 的名字 + 字节偏移
├─────────────────────────────────┤
│  End of Central Directory       │  ← 文件末尾，指向中央目录
└─────────────────────────────────┘
```

读单个文件：`seek(偏移)` → 读压缩块 → Deflate 解压。其余内容完全不碰——这就是随机访问。

### ZIP 为何 35 年仍是主流

1. **1991 年起开放规范**，无专利门槛，各平台原生支持
2. **资源管理器 / Finder 直接当文件夹打开**，零摩擦
3. **大量「换皮 ZIP」**：`.jar`、`.apk`、`.docx`、`.xlsx`、`.epub`、`.war` 本质都是 ZIP
4. **随机访问**很好地满足了「从归档里打开某一个文件」这一最常见场景

ZIP 规范由 PKWARE 以 **APPNOTE.TXT** 维护，近年（约 2020）新增了 zstd（method 93）、XZ（95）、Brotli（98）等压缩方法 ID。也就是说 ZIP 原则上可内嵌 zstd 压缩条目，兼得高压缩率与逐文件随机访问；但 Windows 资源管理器、macOS Finder 与多数解压工具**尚未支持**这些方法——规范跑得比生态快。

### zstd 与随机访问

标准 zstd 是**流式压缩算法**，不是归档格式。`.tar.zst` = tar（收集文件）+ zstd（压缩整个流），与 tar.gz 一样有顺序限制。zstd 有个官方扩展 **Zstandard Seekable Format**，把数据切成独立 Frame 并在末尾放 Seek Table，从而支持随机访问；但主流工具几乎不支持，仍属小众。

### 收敛趋势

ZIP 与 zstd 在互相借鉴：ZIP 引入 zstd 压缩（更高压缩率 + 保留随机访问），zstd 加 Seekable 扩展（获得随机访问）。两者都在朝「高压缩 + 随机访问」收敛。这对矛盾是本质性的——更高压缩率要求更大压缩上下文（Solid），随机访问要求更小的独立块，只能折中选块大小：

| 格式 | 块大小 | 用途 |
| ---- | ------ | ---- |
| **EROFS**（Linux 内核） | 4 KB–1 MB，每块 zstd | Android 系统镜像、容器层 |
| **SquashFS + zstd** | 可配置块 | 嵌入式 Linux、容器镜像 |
| **ZIP + zstd（新）** | 每文件 | 通用归档（尚未广泛支持） |

这些格式服务于「启动时间与内存敏感」的系统（容器、嵌入式、云原生），不太会在通用文件交换中取代 ZIP。

## 维护记录

| 时间 | 修改内容 | 原因 |
| ---- | -------- | ---- |
| 2026-06-27 | 新增「归档格式与随机访问」（ZIP/tar.gz/7z/zstd 对比、ZIP 内部结构、Seekable、EROFS/SquashFS） | 合并 comments-tree 启动打包文档的归档格式章节 |
| 2026-06-24 | 新增「zip 读取到 stdout」小节，补充 `unzip -p` | 查看或管道处理包内文件而不落盘 |
| 2026-06-21 | 新增「zip 列出内容」小节，补充 `unzip -l` / `-v` | 查看压缩包内容而不解压的常用场景 |
| 2026-06-21 | `date` 改回 `2011-11-04`（首条 git commit 中的原始发布日期） | 修正误设为 2026-04-07 的创建日期 |
| 2026-06-21 | 文件重命名为 `compress.md`；title/url 更新；旧 url 加 alias | `compact` 非「压缩」；文件名与内容对齐 |
| 2026-06-21 | title 改为 `Compress and Extract 压缩与解压` | 符合 title 须含英文的规范 |
