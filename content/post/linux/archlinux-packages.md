...
Git 功能的独立可嵌入 C 库，是对 Git 内部代码的 clean-room 重新实现。

**为什么叫 libgit2 而不是 libgit？**

名称中的 "2" 并非版本号，而是与 Git 自身内部代码的命名区分。Git 的源码树中虽然有一些 C 库代码，但从未设计为可被外部链接（没有稳定 API、非线程安全、GPL 无链接例外）。`libgit2` 是全新实现的独立项目，具备：

- 稳定的公共 API
- 线程安全
- GPL with linking exception 授权协议（允许闭源程序使用）

**当前系统中的用途**

`bat` 和 `eza` 通过此库直接读取 Git 仓库状态，无需调用 `git` 命令：

- `bat`：在查看文件时，在行号旁显示行级变更标记（新增 / 修改 / 删除）
- `eza`：使用 `--git` 参数时，在文件列表旁显示每个文件的 Git 状态列

**当前系统被以下包依赖：** `bat`（带语法高亮的 cat 替代工具）、`eza`（现代化 ls 替代工具）、`rust`（Rust 编程语言工具链）。
