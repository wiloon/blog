...
```r
i:          在光标前插入；一个小技巧：按8，再按i，进入插入模式，输入=， 按 esc 进入命令模式，就会出现 8 个=。 这在插入分割线时非常有用，如`30i+<esc>`就插入了 36 个+组成的分割线。
I:          在当前行第一个非空字符前插入；
gI:         在当前行第一列插入；
a:          在光标后插入；
A:          在当前行最后插入；
o           小写 `o` 在当前行的下边插入新行, 在当前行的下边插入一行.
O           大写 `O` 在当前行的上边插入新行.
...
按v进入可视模式
按v进入可视行模式。该模式下文本可以按行来选择。
按Ctrl+v进入可视块模式。该模式下文本可以按照文本块来选择。
进入可视模式当然也标记了你的开始选择点。
...
06 按 `P` 将内容粘贴到光标前面，按 `p` 将内容粘贴到光标后面。
...
- bdw 这也是一个复合命令。b 可以让光标回退到单词开头的位置，而 dw 则是第1个描述过的命令。
- d$ 从光标处删除到行尾
- d1G 删除当前行至文件首行
- dgg 删除至文件首部
- :1,.d 从第1行开始删除到当前行
...
d1G
dgg
# 用命令模式删除从第 1 行到当前行（. 表示当前行）
:1,.d

# 从第一行删除到上一行
# : 进入命令模式
# 1 表示第 1 行
# -1 表示当前行的上一行（. 表示当前行，-1 就是上一行）
# d 表示删除这些行
:1,-1d
