...
存储：Pod 可以指定一组可在各容器之间共用的共享存储卷。持久化通常通过 PVC 挂载；访问模式 **RWO**（ReadWriteOnce）表示同一时刻只有一个节点可读写挂载，与 Longhorn 副本数、Deployment 滚动更新策略有关，详见 [PVC 访问模式：RWO 简介](/pvc-access-modes)。
