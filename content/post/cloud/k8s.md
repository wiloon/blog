...
CRD 向集群注册新的资源类型；Operator 是「CRD + 控制器」，用领域对象（Certificate、Application、PrometheusRule 等）做声明式运维。

详细说明、与 Deployment 的差别、homelab 对应关系与运维注意点，见独立文章 [Kubernetes Operator and CRD](./operator-crd.md)。
...
| 2026-07-13 | CRD/Operator 章节改为概览并链至 `operator-crd.md` | 独立成文便于专题阅读 |
