...
**CloudWatch Alarm 不是独立产品**，而是 CloudWatch 里的「指标超阈值 → 触发动作」能力，常配合 [SNS](./sns.md) 发邮件、或触发 Auto Scaling。
...
> Alarm **只负责「发现问题并通知」**，不负责存失败详情；存档仍靠 [SQS DLQ](./sqs.md) 等组件。
...
备份任务失败链路与 [EventBridge Scheduler](./eventbridge-scheduler.md) 配合时，**DLQ 本身不会发邮件**，需要 CloudWatch Alarm 盯队列深度：
...
[SNS](./sns.md) 邮件订阅首次需在邮箱里 **Confirm subscription**，否则 alarm 触发也收不到信。
