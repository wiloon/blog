...
以 [EventBridge Scheduler](/aws-eventbridge-scheduler) 为例：
...
未配置 DLQ 时，最终失败的消息会被**丢弃**，只能靠 CloudWatch 指标排查。DLQ 有消息后通知人类需配合 [CloudWatch Alarm](/aws-cloudwatch) → SNS，见该文「DLQ 有消息时邮件告警」一节。
