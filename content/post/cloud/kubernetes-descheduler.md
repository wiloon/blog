...
homelab 里曾出现 worker 节点（如 k8s-50）堆了三十多个 Pod，而另一节点几乎空闲的情况。2026-06-23 在 Grafana 上确认 k8s-50 CPU 接近 80% 而 Descheduler 不驱逐，经补 requests、加 **K8s Node Workloads** 看板、放宽 Nexus affinity 后已见效；**全过程**见 [Homelab K8s Node Rebalance 案例](./homelab-k8s-node-rebalance.md)。

当时节点内存压力指标仍为 `MemoryPressure=False`（kubelet 认为内存够用），但 CPU 争抢、有状态服务启动慢等问题已经很明显。手动 `kubectl drain` 后 Pod 能正常分散到其他节点，说明调度器本身没问题，缺的是**主动重平衡**机制。
...
- StatefulSet + RWO PVC：卷挂在哪个节点时 Pod 通常跟过去；Longhorn **多副本**卷在驱逐后**可能** attach 到其他节点，但仍受 nodeAffinity、单副本卷、PDB 等约束
...
3. **显式 nodeAffinity**：例如 Nexus 曾用 `required` 限定 `k8s-50` / `k8s-71`（已改为 `preferred`，见 [案例文](./homelab-k8s-node-rebalance.md)）
4. **资源 requests**：Pod 未设 requests 时，调度器与 `LowNodeUtilization` 都几乎把它当 0；Grafana 上的实际 CPU 与 Descheduler 账本无关。见 [Kubernetes Resource Requests](./kubernetes-resource-requests.md)。
...
| 策略 | 作用 |
| ---- | ---- |
| `LowNodeUtilization` | 按节点 **requests** 占比重平衡；低利用率节点接收、高利用率节点迁出 |
...
阈值含义简述：`thresholds` 以下（且三项都低）视为「低利用率」可接收 Pod；`targetThresholds` 以上（任一项高）视为「高利用率」可迁出 Pod。百分比基于 **requests**，不是 `kubectl top` 实际用量。
...
## DefaultEvictor 与带 PVC 的 Pod

Descheduler v0.36 的 **DefaultEvictor** 用 `podProtections` 控制哪些 Pod 不能被驱逐：

| 配置项 | 作用 |
| ------ | ---- |
| `defaultDisabled` | 关闭**默认**保护（如 `PodsWithLocalStorage` → 允许驱逐 emptyDir 等本地卷 Pod） |
| `extraEnabled` | 开启**额外**保护；`PodsWithPVC` 在此列表，**默认并不保护 PVC Pod** |

homelab 当前只 `defaultDisabled: PodsWithLocalStorage`，**没有** `extraEnabled: PodsWithPVC`，因此带 PVC 的 Deployment Pod **可以被** DefaultEvictor 选中驱逐——前提是策略判定该节点 overutilized，且 Pod 没有 nodeAffinity 等硬约束。

若只想保护部分 StorageClass（例如单副本 `longhorn-single`），可显式启用并按 SC 过滤：
...
Nexus 使用 Longhorn **2 副本**卷；曾用 `required` nodeAffinity 限定 k8s-50 / k8s-71，后改为 **preferred**，Descheduler 驱逐或 Deployment 滚动后 Pod 可落到 k8s-71（现网已验证 attach）。不会落到无卷副本的 k8s-21 / k8s-39，除非 Longhorn 后续扩 replica。

建议同时开启 `nodeFit: true`（homelab 已开）：驱逐前做调度器 dry-run，避免迁到放不下该 Pod 的节点。

## 部署前注意

1. **先设 Pod `resources.requests`**，否则利用率策略缺乏可靠依据（详见 [Resource Requests](./kubernetes-resource-requests.md)）
...
- [Homelab K8s Node Rebalance: 从 Grafana 发现到 Descheduler 生效](./homelab-k8s-node-rebalance.md) — 发现、监控、治理、见效全过程
- [Kubernetes Resource Requests: 调度与 Descheduler 在算什么](./kubernetes-resource-requests.md) — requests / limits / 实际用量
...
| 时间 | 修改内容 | 原因 |
| ---- | -------- | ---- |
| 2026-06-23 | 补充 DefaultEvictor / PVC 保护说明；链到 resource-requests 文 | Descheduler 未触发排查与 v0.36 podProtections 行为澄清 |
| 2026-06-23 | 链到 homelab-k8s-node-rebalance 案例；更新 Nexus affinity 现网描述 | 问题已解决，记录完整过程 |
