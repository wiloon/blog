...
AVL 树是最早出现的自平衡二叉搜索树，由 G.M. Adelson-Velsky 和 E.M. Landis 于 1962 年在论文《An algorithm for the organization of information》中提出，因此得名 AVL（发明者姓氏首字母），也叫高度平衡树。

平衡二叉树的通用定义（平衡因子、失衡、单旋恢复平衡的例子）见 [二叉树与二叉搜索树 §平衡二叉树 Balanced Binary Tree](./binary-tree.md#平衡二叉树-balanced-binary-tree)，本文只讲 AVL 特有的部分：严格平衡条件、四种失衡情况的旋转、插入删除的复杂度，以及和其他平衡树、常见平衡树家族的关系。
...
RR 单旋的完整示例见 [二叉树与二叉搜索树 §插入示例](./binary-tree.md#插入示例从平衡到失衡再到旋转)，LL 是它的镜像。下面补充一个 LR 双旋的例子。
...
AVL 树和红黑树都属于自平衡二叉搜索树这个家族。同一层里工程上还会碰到伸展树（Splay Tree）和树堆（Treap）。家族关系、和普通 BST 的差别，以及 B 树 / B+ 树为什么不算二叉实现，见 [二叉树与二叉搜索树 §自平衡二叉搜索树](./binary-tree.md#自平衡二叉搜索树)。
...
| 时间 | 修改内容 | 原因 |
| ---- | -------- | ---- |
| 2026-07-14 | 由 `inbox/平衡二叉树.md` 整理为正式文章，重命名为 `avl-tree.md`；补充树高维护、四种失衡情况与旋转（含 LR 双旋图解）、插入删除复杂度、与红黑树对比的内链等内容；原文缺失的示意图改为 Mermaid 图 | 原文是 inbox 目录下未整理的草稿，内容不完整（多处提到"看下图"但配图缺失）、格式不规范；整理后与 [二叉树与二叉搜索树](./binary-tree.md)、[Red-Black Tree](./red-black-tree.md) 形成完整的平衡树系列 |
| 2026-09-15 | 内链与「自平衡二叉搜索树」称谓随 [二叉树与二叉搜索树](./binary-tree.md) 统一 | 系列文章译名对齐，避免标题锚点失效 |
