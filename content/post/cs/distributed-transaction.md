...
从架构层面上看，在需要接入第三方的场景，不可能让第三方也配合你实现 TCC 的三个接口。SAGA 可以在只依赖正向操作与补偿（回滚）操作的前提下解决这一问题。
SAGA 只需要正向操作和补偿（逆向）操作。以第三方转账/扣款为例：本地生成订单的同时调用银行的扣款接口（正向）；如果订单生成失败（子事务失败），则调用补偿操作（例如退款接口）。这种方式对用户不够友好，因为部分服务的正向操作可能已生效，用户会短暂看到中间状态。

1. 协调式，需要开源协调器统一处理。譬如seata
2. 事件驱动式(依赖MQ)，业务层代码通过调用消息队列自己处理
