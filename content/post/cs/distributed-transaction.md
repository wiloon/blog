...
从架构层面上看，在需要接入第三方的场景，不可能让第三方也配合你实现TCC的三个接口。saga解决了这个问题。
saga只需要**占用**（正向）和 **回滚**（逆向）两个操作。还是第三方转账、扣款的例子， 本地生成订单的同时调用银行的扣款接口（正向）， 如果订单生成失败（子事务失败）， 调用回滚操作， 也就是给用户支付款项的接口，这并不需要银行提供什么额外的接口就能实现， 但是用户不友好，因为部分服务的正向操作已经生效， 用户会看到。
