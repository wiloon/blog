...
title: 缓存
author: "-"
date: 2026-04-14T21:41:45+08:00
url: cache
categories:
  - inbox
tags:
...
在高并发系统中缓存穿透，如果一个 req 需要请求的 key 在缓存中没有，这时业务线程就会访问磁盘数据库系统，然而磁盘数据库也没有这个 key，无奈业务线程只能返回 null，白白处理一圈。
...
如果一个查询返回的数据为空（不管是数据不存在，还是系统故障），仍然把这个空结果进行缓存，但它的过期时间会很短，不超过 5 分钟。通过这个直接设置的默认值存放到缓存，这样第二次到缓存中获取就有值了，而不会继续访问数据库。当修改或者新增该 key 的数据信息的时候，需要删除或者更新 null 缓存值。
...
- 优点：内存占用极小，拦截效果好
- 缺点：有一定误判率，且不支持删除（可用 Counting Bloom Filter 解决）
...
生产环境通常两者结合：**接口层校验 + 布隆过滤器**，布隆过滤器作为最后一道防线。

### 缓存击穿 Hotspot Invalid

**单个**热点 key 过期失效，此时恰好有大量并发请求访问该 key，请求全部穿透缓存打到数据库，造成瞬时数据库压力骤增。

与缓存雪崩的区别：

| 维度 | 缓存击穿 | 缓存雪崩 |
|------|---------|---------|
| 失效 key 数量 | 单个（或少量）热点 key | 大量 key 同时失效 |
| 触发原因 | 热点 key 自然过期 | 大量 key 同时过期 / 缓存服务宕机 |
| 影响范围 | 局部压力骤增 | 全量请求涌入，系统性崩溃 |
| 严重程度 | 较轻 | 更严重 |
...
保证在分布式情况下, 使用分布式锁保证对于每个 key 同时只允许只有一个线程查询到后端服务, 其他没有获取到锁的权限, 只需要等待即可；
这种高并发压力直接转移到分布式锁上, 对分布式锁的压力非常大。
获取到锁的请求将数据写入成功到 redis 中, 通知没有获取锁的请求直接从 Redis 获取数据即可
