...
lastmod: 2026-05-14T15:31:40+08:00
url: inotify
categories:
  - Linux
tags:
...
内核只负责将事件写入 inotify fd，至于如何读取，由应用程序自行决定。常见方式有三种：

- **阻塞 `read`**：直接调用 `read(fd)`，没有事件时线程阻塞，有事件时返回。简单但会占用一个线程
- **`select` / `poll` / `epoll`**：将 inotify fd 加入 I/O 多路复用，只有 fd 可读时才处理事件，适合同时监听多个 fd。大多数框架底层使用这种方式
- **带超时的轮询**：使用带超时参数的 `poll`，定期检查，不常用

**框架层面的"回调"机制**

实际开发中常见的"注册回调、事件触发"体验，是由上层库封装的：

- libuv（Node.js 底层）、GLib、libevent 等事件循环库内部使用 epoll 监听 inotify fd
- 对上层暴露回调接口，如 Node.js 的 `fs.watch()`、Go 的 `fsnotify` 库

所以"回调机制"是库/框架帮你封装的，Linux 内核本身只提供 fd 这一接口。
