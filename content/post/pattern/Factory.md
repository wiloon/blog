...
下面我们从[开闭原则](../other/open-closed-principle.md) (对扩展开放；对修改封闭) 上来分析下简单工厂模式。
...
随着车型不断增加，`Driver` 类需要了解所有车型的创建细节，承担了过多的职责，这样的类在设计模式中称为**上帝类（God Class）**——什么都管、什么都知道，最终变得难以维护。这也违反了[单一职责原则](./solid-principles.md)（一个类只应承担一个职责）。
