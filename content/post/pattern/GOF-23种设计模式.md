...
7. 适配器(Adapter)模式：将一个类的接口转换成客户希望的另外一个接口，使得原本由于接口不兼容而不能一起工作的那些类能一起工作。[[适配器模式-adapter#适配器模式 Adapter]]
...
3.JavaEE只是适合企业计算应用的框架软件，但是GoF的设计模式几乎可以用于任何应用！因此GoF的设计模式应该是JavaEE的重要理论基础之一。

所以说，GoF的设计模式是Java基础知识和JavaEE框架知识之间一座隐性的"桥"。为什么说隐性的？

GOF设计模式是一座隐性的"桥"

因为很多人没有注意到这点，学完Java基础语言就直接去学J2EE,有的甚至鸭子赶架，直接使用起Weblogic等具体JavaEE软件，一段时间下来，发现不过如此，挺简单好用，但是你真正理解JavaEE了吗？你在具体案例中的应用是否也是在延伸JavaEE的思想？

如果你不能很好的延伸JavaEE的思想，那你岂非是大炮轰蚊子，认识到JavaEE不是适合所有场合的人至少是明智的，但我们更需要将JavaEE用对地方，那么只有理解JavaEE此类框架软件的精髓，那么你才能真正灵活应用Java解决你的问题，甚至构架出你自己企业的框架来。(我们不能总是使用别人设定好的框架，为什么不能有我们自己的框架？)
