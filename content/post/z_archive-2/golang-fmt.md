...
– 左对齐, 备用格式: 为八进制添加前导 0 (%#o) ,为十六进制添加前导 0x (%#x) 或0X (%#X) ,为 %p (%#p) 去掉前导 0x；对于 %q,若 strconv.CanBackquote 返回 true, 就会打印原始 (即反引号围绕的) 字符串；如果是可打印字符, %U (%#U) 会写出该字符的 Unicode 编码形式 (如字符 x 会被打印成 U+0078 'x') 。
