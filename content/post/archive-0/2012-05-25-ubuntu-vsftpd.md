...
将local\_enable=YES前的#去掉，使其可用，表示允许本地用户登录；增加listen\_port=21配置项，设置监听端口为21；该步骤可选，不允许匿名用户登录，将anonymous_enable=YES的值改为NO，根据实际需要设置。
...
setsebool ftpd\_disable\_trans 1 是停用SELinux 對ftpd 的保護，這樣，ftpd 就可以跟平常一樣的運作了。
