...
<pre><code class="language-bash line-numbers"># 可以用 -X POST 来申明我们的请求方法，用 -d 参数，来传送我们的参数。
curl -d "user=admin&passwd=12345678" http://127.0.0.1:8080/login

# POST请求
curl -i -XPOST 'http://localhost:8186/write?db=telegraf' --data-binary @foo
# foo 是一个包含请求数据的文件
</code></pre>

### use socks5 proxy

<pre><code class="language-bash line-numbers">curl -x socks5h://localhost:8001 http://www.google.com/
curl -x socks5://localhost:8888 http://google.com
</code></pre>

### url encoding

<pre><code class="language-bash line-numbers"># --data-urlencode 要跟 -G 配合使用
...
<pre><code class="language-bash line-numbers">curl http://grafana.wiloon.com -v --retry 0 --connect-timeout 1
