...
### 经量级锁
所有现代JVM都引入了经量级锁: 避免将每个对象关联操作系统的 mutex/condition 变量 （重量级锁）
...
2. 将锁对象头中的Mark Word拷贝到当前线程的Lock Record中，称为Displaced Mark Word，然后使用CAS，将对象头中的Mark Word修改为指向当前线程栈中Lock Record的指针。如果成功，则获取轻量级锁，执行同步块中的代码，如果失败，则进行自旋竞争锁，自旋达到一定的次数如果依旧没有获取到锁，则升级为重量级锁（因为自旋会消耗CPU，为了避免无用的自旋，一旦锁升级为重量级锁，就不会恢复到轻量级锁，自旋的线程会被挂起阻塞住）；
