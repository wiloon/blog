...
要得到正在执行的程序/脚本自身所存放的绝对路径，在 PHP 里面可以用 <span style="font-family: 'courier new', courier, monospace;"><span style="color: #ff0000;">dirname(<span style="color: #ff0000;">realpath(<span style="color: #0000ff;">__FILE__)) ； C# 则有 <a href="http://msdn2.microsoft.com/en-us/library/system.windows.forms.application.startuppath.aspx" target="_blank">System.Windows.Forms.Application.StartupPath</a> ； java 似乎没有什么比较直接的方法，只能[利用 CodeSource 来间接获取][1] 。而在 linux shell 脚本里面如果想得到当前脚本文件存放的绝对路径，也没有太现成的命令可以调用，不过可以通过下面的语句来获取：

<span style="font-family: 'courier new', courier, monospace;">baseDirForScriptSelf=$(cd "$(dirname "$0")"; pwd)
