...
目录
  ...
  7. 引出
    ...
    详细实现代码在normal\_hash\_add.py输出是这样的：
...
我们依然对其进行了实现consist\_hash\_add.py，并且观察了数据迁移的结果：
...
consist\_hash\_virtual
  
详细实现请见virtual\_consist\_hash.py
...
同时，观察增加节点后数据变动情况，详细的代码请见virtual\_consist\_hash_add.py：
...
可以看到，数据分布是比较理想的。如果节点数刚好和分区数相等，理论上是可以均匀分布的。而观察下增加节点后的数据移动比例，代码实现见part\_consist\_hash.py
