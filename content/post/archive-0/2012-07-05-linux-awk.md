...
<pre><code class="language-bash line-numbers">cat foo.log| awk -F 'data:' '{print $2}'
</code></pre>

### 字段非空

<pre><code class="language-bash line-numbers">awk '{if(NR&gt;3 && $2 != "") {print $2}}'|sort -u
</code></pre>
...
<pre><code class="language-bash line-numbers">awk '{print "\""}'
</code></pre>
...
<pre><code class="language-bash line-numbers">echo "i have two apples and one banana" | awk -F'one|two' '{for(i=1;i&lt;=NF;i++)print i,"="$i}'
# 1 =i have
# 2 = apples and
# 3 = banana
</code></pre>
...
<pre><code class="language-bash line-numbers">substr($4,20)     ---&gt;  表示是从第4个字段里的第20个字符开始，一直到设定的分隔符","结束.
...
<pre><code class="language-bash line-numbers">tail -n 10000 info.log | grep 'xxx.*Unknown.*LF'|awk -F 'Unk|:L' '{print substr($2,14,7),substr($3,0,16)}'
</code></pre>

### 统计

<pre><code class="language-bash line-numbers"># 平均
...
<pre><code class="language-awk line-numbers">#!/bin/awk -f
#运行前
BEGIN {
    printf "begin0\n"
    FS="Unk|:L"
...
<blockquote class="wp-embedded-content" data-secret="7pUo5sdZvt">
  <p>
    <a href="https://coolshell.cn/articles/9070.html">AWK 简明教程</a>
  </p>
</blockquote>

<iframe title="《AWK 简明教程》—酷 壳 - CoolShell" class="wp-embedded-content" sandbox="allow-scripts" security="restricted" style="position: absolute; clip: rect(1px, 1px, 1px, 1px);" src="https://coolshell.cn/articles/9070.html/embed#?secret=7pUo5sdZvt" data-secret="7pUo5sdZvt" width="600" height="338" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>
