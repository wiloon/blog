...
```bashcat foo.log| awk -F 'data:' '{print $2}'
```

### 字段非空

```bashawk '{if(NR&gt;3 && $2 != "") {print $2}}'|sort -u
```
...
```bashecho "i have two apples and one banana" | awk -F'one|two' '{for(i=1;i&lt;=NF;i++)print i,"="$i}'
# 1 =i have
# 2 = apples and
# 3 = banana
```
...
```bashsubstr($4,20)     ---&gt;  表示是从第4个字段里的第20个字符开始，一直到设定的分隔符","结束.
...
```bashtail -n 10000 info.log | grep 'xxx.*Unknown.*LF'|awk -F 'Unk|:L' '{print substr($2,14,7),substr($3,0,16)}'
```
