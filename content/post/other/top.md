...
lastmod: 2026-05-24T17:22:34+08:00
url: top
categories:
  - Linux
tags:
...
- 2 users:  当前两个用户在线
- load average: 0.00,0.00,0.00:  系统 1 分钟, 5 分钟, 15 分钟的 CPU 负载信息
...
在 macOS 的活动监视器或 iStat Menus 里，经常能看到 `System`、`User`、`Idle` 这三个指标。它们和 `top` 的字段可以直接对应：

- System = `sy`，内核态（Kernel space）消耗的 CPU 时间占比
- User = `us`，用户态（User space）消耗的 CPU 时间占比
- Idle = `id`，空闲 CPU 时间占比

这里统计的是**时间占比**，不是「有多少个内核线程 / 用户线程」。采样窗口内，所有核心上实际花在用户态、内核态上的 CPU 时间分别加总，再算成百分比。

同一个线程在执行过程中，会不断在用户态和内核态之间切换，并不是固定只属于 User 或 System。例如：

1. 线程在用户态跑应用代码 → 计入 User
2. 发起系统调用（读文件、发网络包、分配内存触发缺页等）→ CPU 进入内核态处理 → 计入 System
3. 系统调用返回 → 回到用户态继续执行 → 又计入 User

所以「前半段在用户态、后半段在内核态」完全可能；实际里往往是很多次短切换，而不一定正好对半。常见触发切换的情况包括系统调用、缺页中断、硬件中断、调度器换线程等。用户态不能直接访问硬件或随意改页表，需要内核介入时就会发生特权级切换（例如 x86 的 ring 3 ↔ ring 0）。

一次 `read()` 读文件时：应用在用户态准备参数是 User；进入内核做 VFS、块 I/O 的那段是 System；返回用户态拷贝数据又是 User。磁盘、网络忙时 System 往往会升高，即使用户程序本身 CPU 不重。

`System` 常见来源是内核调度、文件系统、网络栈、驱动和中断处理。日常观察时，通常优先看总 CPU 占用和机器是否卡顿，不需要单独盯 `System`。但如果 `System`（或 `sy`）长期偏高，同时机器出现明显卡顿，再进一步排查磁盘 I/O、网络、驱动或系统服务会更有效。
...
进程使用的虚拟内存总量，单位 KiB (按 f 键能看到 VIRT的单位是KiB), ubuntu 22.04 的 top 默认会显示单位.
VIRT: virtual memory usage。Virtual 这个词很神，一般解释是: virtual adj.虚的, 实质的, [物]有效的, 事实上的。
到底是虚的还是实的？让 Google 给 Define 之后，将就明白一点，就是这东西还是非物质的，但是有效果的，不发生在真实世界的，发生在软件世界的等等。
...
单位 KiB
DATA: 数据占用的内存。如果 top 没有显示，按 f 键可以显示出来。这一块是真正的该程序要求的数据空间，是真正在运行中要使用的。
