...
title: 微服务
author: "-"
date: 2026-04-25T11:32:20+08:00
url: microservice
categories:
  - cloud
tags:
...
线上突发大流量，首要目标是**保证业务响应**，处理优先级如下：

### 第一步：立即扩容（Scale Out）

扩容是最直接的手段，先扛住流量再说。增加 CPU、内存和服务实例数，让系统有能力处理当前请求量，避免超时和雪崩。K8s HPA 可以根据负载自动触发水平扩容。

> 扩容解决的是「服务撑不住」的问题，是救火的第一步。

### 第二步：限流与熔断（防止崩溃）

扩容的同时在入口处限流（令牌桶/漏桶），对不稳定的下游依赖启用熔断，防止级联故障扩散。

### 第三步：消息队列（削峰填谷，异步化）

消息队列是应对流量洪峰的长期方案，将请求异步化，缓冲突发流量，平滑写入压力。但引入消息队列需要改造业务逻辑，不适合作为紧急应对手段。

> 消息队列解决的是「流量不均匀」的问题，是架构层面的优化。

### 第四步：缓存

热点数据缓存（Redis 等）减少 DB 压力，降低单次请求的资源消耗。

### 总结

| 手段 | 适用时机 | 目标 |
|------|----------|------|
| 扩容 | 流量突发，立即执行 | 先扛住请求，保证响应 |
| 限流/熔断 | 同步进行 | 防止系统崩溃 |
| 消息队列 | 架构优化阶段 | 异步削峰，长期稳定 |
| 缓存 | 读多写少场景 | 降低 DB 压力 |
