...
Duration表示Instant之间的时间差,可以用来统计任务的执行时间,也支持各种运算操作,比如:
...
1. LocalDate 与 Period
2. LocalDate用于表示日期,与时区(TimeZone)无关。
...
2. LocalTime表示时间,没有日期,与时区(TimeZone)无关:

LocalTime.now().isBefore(LocalTime.of(16, 2, 1));
  
LocalTime.now().plusHours(2..getHour();
  
### LocalDateTime表示日期和时间,适用于时区固定不变的场合(LocalDateTime使用系统默认的时区),如果需要根据时区调整日期和时间,应该使用ZonedDateTime
