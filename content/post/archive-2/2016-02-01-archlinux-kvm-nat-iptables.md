...
<pre><code class="language-bash line-numbers">egrep --color=auto 'vmx|svm|0xc0f' /proc/cpuinfo
...
<pre><code class="language-bash line-numbers">sudo pacman -S qemu libvirt
...
<pre><code class="language-bash line-numbers">sudo iptables -t nat -A POSTROUTING -o wlp3s0 -j MASQUERADE
</code></pre>

### 安装 win10

<pre><code class="language-bash line-numbers">qemu-system-x86_64 \
-enable-kvm \
-cpu host \
-m 2048 \
-boot d \
...
<pre><code class="language-bash line-numbers">chmod 777 /usr/share/virtio/virtio-win_x86_64.vfd
</code></pre>
...
<pre><code class="language-bash line-numbers">#start kvm with virtio net (install eth)
qemu-system-x86_64 \
 -enable-kvm \
 -cpu host \
 -m 2048 \
...
<pre><code class="language-bash line-numbers">#User-mode networking
