...
# perf top -e kmem:kmem\_cache\_alloc
...
4.86% \[kernel\] \[k\] \_spin\_lock
       
2.43% \[kernel\] \[k\] \_spin\_lock_bh
       
2.29% \[kernel\] \[k\] \_spin\_lock_irqsave
       
1.77% \[kernel\] \[k\] _\_d\_lookup
       
1.55% libc-2.12.so [.] _\_strcmp\_sse42
...
需要编译选项的支持：CONFIG\_LOCKDEP、CONFIG\_LOCK_STAT。

CONFIG_LOCKDEP defines acquired and release events.

CONFIG\_LOCK\_STAT defines contended and acquired lock events.
...
-k <value>：sorting key，默认为acquired，还可以按contended、wait\_total、wait\_max和wait_min来排序。
