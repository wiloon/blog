...
<pre><code class="language-sql line-numbers">SELECT DATE('2017-02-11')
SELECT DATE('2017/02/11')
SELECT STR_TO_DATE('2015/02/25','%Y/%m/%d')
SELECT STR_TO_DATE('2015-02-25','%Y-%m-%d')
```

<pre><code class="language-sql line-numbers">select count(*)/60 from xxx
...
<pre><code class="language-sql line-numbers">select DATE_SUB(NOW(),INTERVAL  10 SECOND)
...
在MySQL 3.23中，你可以使用+和-而不是DATE\_ADD()和DATE\_SUB()。（见例子）date是一个指定开始日期的
...
MySQL假设日子和小时部分是丢失的并且值代表分钟和秒。换句话说，&#8221;1:10&#8243; DAY\_SECOND以它等价于&#8221;1:10&#8221; MINUTE\_SECOND
...
mysql> select FROM\_UNIXTIME(UNIX\_TIMESTAMP(),
