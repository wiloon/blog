...
echo "${TIME\_STAMP}: Could not open password file \"${PASSFILE}\" for reading." >> ${LOG\_FILE}
...
echo "${TIME\_STAMP}: User does not exist: username=\"${username}\", password=\"${password}\"." >> ${LOG\_FILE}
...
echo "${TIME\_STAMP}: Successful authentication: username=\"${username}\"." >> ${LOG\_FILE}
    
exit 0
  
fi

echo "${TIME\_STAMP}: Incorrect password: username=\"${username}\", password=\"${password}\"." >> ${LOG\_FILE}
