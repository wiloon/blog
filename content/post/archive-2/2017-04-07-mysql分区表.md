...
<code class="language-sql line-numbers">ALTER TABLE table0 PARTITION BY RANGE (to_days(field0)) (
...
<code class="language-sql line-numbers">alter TABLE  table0 add PARTITION ( PARTITION p201908 VALUES less THAN (737668));
alter table table0 add PARTITION ( PARTITION pmax VALUES less than MAXVALUE);
```

<code class="language-sql line-numbers">-- check partition size
