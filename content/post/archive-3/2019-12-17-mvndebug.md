...
<pre><code class="language-bash line-numbers">mvnDebug jetty:run
# 默认调试端口8000
</code></pre>

maven的安装目录下存在 mvnDebug.bat 文件，打开可以看到具体的配置项如下：

<pre><code class="language-bat line-numbers">set MAVEN_DEBUG_OPTS=-Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=8000
</code></pre>
...
<pre><code class="language-bash line-numbers">Application prints "Listening for transport dt_socket at address: 8000" and does not halt
</code></pre>
