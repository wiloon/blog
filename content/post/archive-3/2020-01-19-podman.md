...
```bashpodman generate systemd container0 &gt; /usr/lib/systemd/system/container0.service
```

### network

```bashpodman network  create --driver bridge net0
...
```bashsudo dnf install https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
```
...
```bashpodman volume create volume0
