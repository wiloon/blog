...
## 用 --add-dir 实现类似 multi-root workspace

Claude Code 默认只能访问启动时所在的目录。`--add-dir` 可以额外授权访问其他目录，效果类似 VS Code 的 multi-root workspace（同时打开多个项目目录）：

```bash
claude --add-dir ../other-project --add-dir ../shared-lib
```

可以叠加多个 `--add-dir`，每个都会被加入可访问目录白名单（同时也会加载这些目录下的 CLAUDE.md）。
