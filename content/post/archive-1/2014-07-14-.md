...
需要注意，为了使用该设备，你需要 /usr/lib/alsa-lib/libasound\_module\_pcm\_bluetooth.so 这一个蓝牙plugin的库文件。这是在Bluez相关的包里，和Alsa本身没有关系。从这里，我们也可以看出alsa的外部plugin和配置文件之 间的名字关系规则： libasound\_module\_pcm\_####.so 这里的#### 就是你再conf文件中pcm.xxxx 里所写的名字。
