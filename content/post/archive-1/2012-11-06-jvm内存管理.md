...
http://www.infoq.com/cn/articles/Troubleshooting-Java-Memory-Issues?utm\_campaign=infoq\_content&utm\_source=infoq&utm\_medium=feed&utm_term=global
...
<blockquote data-secret="7jR72YAJQL" class="wp-embedded-content">
  
    <a href="http://blog.wiloon.com/?p=11692">Eclipse Memory Analyzer</a>
  
</blockquote>

<iframe class="wp-embedded-content" sandbox="allow-scripts" security="restricted" style="position: absolute; clip: rect(1px, 1px, 1px, 1px);" src="http://blog.wiloon.com/?p=11692&#038;embed=true#?secret=7jR72YAJQL" data-secret="7jR72YAJQL" width="600" height="338" title=""Eclipse Memory Analyzer&#8221; &#8212; w1100n" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>

NMT, pmap
  
Max memory = [-Xmx] + [-XX:MaxPermSize] + number\_of\_threads * [-Xss]
...
reserved memory 是指JVM 通过mmap PROT\_NONE 申请的虚拟地址空间，在页表中已经存在了记录（entries），保证了其他进程不会被占用，会page faults, committed memory 是JVM向操做系统实际分配的内存（malloc/mmap）,mmaped PROT\_READ | PROT_WRITE,仍然会page faults 但是跟 reserved 不同，完全内核处理像什么也没发生一样。 used memory 是JVM实际存储了数据（Java对象）的大小，当used~=committed的时候，heap就会grow up，-Xmx设置了上限。
...
Thus useful in implementing protection for areas such as network interfacing, virtual machines, and interpreters. An example usage: pthread\_attr\_setguardsize, pthread\_attr\_getguardsize.
...
http://timyang.net/java/java\_gc\_tunning/
