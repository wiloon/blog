...
ww.51testing.com/html/28/116228-238479.html
...
当其具体内容是${\_\_CSVRead(${\_\_property(user.dir)}${FILENAME},next())}时用来模拟读文件的下一行。
...
${\_\_CSVRead(data.txt,1)}${\_\_CSVRead(data.txt,next)}&#8212;读取本行的第二列值，并把行标移动到下一行
...
${\_\_log(${\_\_CSVRead(data.txt,1)})}这样的写法是错误的，JMeter会抛出异常，该怎么办呢？
