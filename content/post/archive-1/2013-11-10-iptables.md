...
<pre><code class="language-bash line-numbers">Synopsis
...
<pre><code class="language-bash line-numbers">iptables -t filter -L FORWARD
</code></pre>

iptalbes会使用nf_conntrack模块跟踪连接，而这个连接跟踪的数量是有最大值的，当跟踪的连接超过这个最大值，就会导致连接失败。 通过命令查看

<pre><code class="language-bash line-numbers"># 当前值
...
<pre><code class="language-bash line-numbers"># 按line number删除
...
<pre><code class="language-bash line-numbers">#查看定义规则的详细信息
...
### archlinux, if you want iptables to be loaded automatically on boot, you must enable iptables.service

<pre><code class="language-bash line-numbers"># save iptable rules in iptables.rules
iptables-save &gt; /etc/iptables/iptables.rules
</code></pre>

<pre><code class="language-bash line-numbers">iptables [-t tables] &lt;-A/I/D/R&gt; 规则链名 [规则号] &lt;-i/o 网卡名&gt; -p 协议名 &lt;-s 源IP/源子网&gt; --sport 源端口 &lt;-d 目标IP/目标子网&gt; --dport 目标端口 -j 动作
# tables 表名
</code></pre>
...
<pre><code class="line-numbers"><br /> 当然你如果想拒绝的更彻底：
...
http://xstarcd.github.io/wiki/Linux/iptables\_forward\_internetshare.html

 [1]: https://s2.ax1x.com/2020/01/31/11HP74.md.png
 [2]: https://imgchr.com/i/11HP74
 [3]: https://s2.ax1x.com/2020/01/31/11bp8I.md.png
 [4]: https://imgchr.com/i/11bp8I
 [5]: https://s2.ax1x.com/2020/01/31/11v2WQ.md.png
 [6]: https://imgchr.com/i/11v2WQ
 [7]: https://s2.ax1x.com/2020/01/31/13pGXd.png