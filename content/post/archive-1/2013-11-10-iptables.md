...
```bashiptables -t filter -L FORWARD
```
...
```bash# save iptable rules in iptables.rules
iptables-save &gt; /etc/iptables/iptables.rules
```

```bashiptables [-t tables] &lt;-A/I/D/R&gt; 规则链名 [规则号] &lt;-i/o 网卡名&gt; -p 协议名 &lt;-s 源IP/源子网&gt; --sport 源端口 &lt;-d 目标IP/目标子网&gt; --dport 目标端口 -j 动作
# tables 表名
```
