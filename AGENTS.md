...
- **内容约束（语气平实、VPN 命名、代码块注释用英文等）**：见 [.ai/content-constraints.md](.ai/content-constraints.md)
...
技术文章**过长**（经验值 > 800 行，或明显涵盖多个独立主题）时，AI **不要擅自拆分**——拆分会改动 URL、内链与归属，属于 scope 变化。应**主动提示作者**并给出拆分方案（哪些章节 → 哪个新文件 / 已有文件），由作者确认后再执行。行数参考与 hub 拆分原则见 [.ai/doc-length-and-split.md](.ai/doc-length-and-split.md)。
...
6. ✅ **编辑内容** → 完成实际的文章修改
7. ✅ **维护记录** → 仅当文章 `date` **不是今天**（即已发布文章）时，才在文章末尾添加 `## 维护记录` 表格；若表格中**已有今天日期**的行则跳过不再更新；今天新建并首次发布的文章**不需要**维护记录
...
- 🔧 **必须使用工具获取准确时间**（不要猜测）。机器本地时区可能是 PDT/UTC 等，**必须先换算成 Asia/Shanghai 再写入**，不能把本地钟面数字直接拼上 `+08:00`：
  - ✅ 推荐：`bash scripts/blog-now.sh`（输出完整时间戳）或 `bash scripts/blog-now.sh --date`（仅 `YYYY-MM-DD`，用于判断「是否今天」）
  - ✅ 等价一行：`TZ=Asia/Shanghai date '+%Y-%m-%dT%H:%M:%S+08:00'`
  - ❌ 禁止：`date '+%Y-%m-%dT%H:%M:%S+08:00'`（只改后缀、不换算；本机若在 UTC-7，会把 `01:36` 错写成 `01:36+08:00`，实际东八区应是 `16:36+08:00`）
...
- 「今天」以**东八区日历日**为准：用 `bash scripts/blog-now.sh --date` 与 `lastmod` / `date` 的日期部分比较，不要用本机本地日期
...
8. ✅ **内容编辑**：完成实际的文章内容修改
9. ✅ **维护记录**：仅当文章 `date` **不是今天**（已发布文章）时，在文章末尾添加 `## 维护记录` 表格（见下文规范）；若表格中**已有今天日期**的行则跳过不再更新；今天新建并首次发布的文章**不需要**维护记录
10. ✅ **格式检查**：确保 Markdown 格式正确

**记忆口诀：编辑文章 = 改文件名 + 改标题 + 检查 URL + 站内互链 + 检查分类 + 按需更新 lastmod（不改 date；同日不重复）+ 加标签 + 改内容 + 维护记录（仅已发布文章；同日已有记录则跳过）**

### 维护记录规范

**核心规则：仅对已发布文章（`date` 不是今天）进行 AI 辅助编辑后，才在文章末尾添加 `## 维护记录` 章节；若当天已有记录则跳过，不重复更新。今天新建并首次发布的文章不需要维护记录。**
...
- 原因：说明为什么要改
- **判断是否需要维护记录**：检查 front matter 的 `date` 字段，若日期是今天则跳过；若是过去日期则需要追加（但若当天已有记录，见下方「新增 vs 跳过」）
- **新增 vs 跳过**：
  - 若表格中**没有今天日期**的行 → 在末尾追加一行新记录，**不要删除历史记录**
  - 若表格中**已有今天日期**的行（同一天内已经维护过）→ **跳过，不再更新维护记录**，保留已有的当天记录原样不动
...
Cloudflare Pages 在构建失败时会**保留上一次成功的部署**：站点 `https://wiloon.com/` 仍可访问、表面无异常，但内容停在旧版本——新文章不会出现。作者已在 Cloudflare 配置了构建失败邮件提醒，因此**不再需要 AI 在本地跑构建自查**，出问题会通过邮件收到通知。
...
- ✅ 编辑或新建文章后，按上文规则给含特殊字符的 `title` 加引号即可，**不需要**再本地跑 `hugo`/`task preview` 验证——构建失败会有 Cloudflare 邮件提醒。
- ✅ 如需批量排查存量文章，可一次性扫描所有「标题含冒号却未加引号」的文章：
