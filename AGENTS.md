...
## 🚨 强制约束：带 `original` 标签的文档 🚨

带有 `original` 标签的文档是作者本人创作的原创内容，AI agent **禁止**修改其正文文字部分，除非有来自用户的明确指令。

**允许的操作：**

- 修正标点符号的使用（如半角/全角、书名号等）
- 检查语法错误并提出建议
- 更新 front matter 字段（date、tags 等）
...
**记忆口诀：编辑文章 = 改文件名 + 改标题 + 检查 URL + 检查分类 + 改日期 + 加标签 + 改内容**
