...
4. ✅ **更新 lastmod 字段** → 更新或添加 `lastmod` 为当前东八区时间；`date` 字段保留原始创建日期，**不要修改**
5. ✅ **添加标签** → 先检查是否有 `original` 标签：有则只加 `AI-assisted` 并删除 `remix`/`reprint`；无则加 `remix` 和 `AI-assisted` 并删除 `reprint`
6. ✅ **编辑内容** → 完成实际的文章修改
7. ✅ **维护记录** → 在文章末尾添加或更新 `## 维护记录` 表格，记录本次改动的时间、内容和原因
...
lastmod: 2025-10-30T08:30:00+08:00  # 最后修改日期，每次编辑更新
url: article-url
categories:
  - 分类
tags:
...
lastmod: 2025-10-30T08:30:00+08:00  # ✅ 更新为当前时间
url: article-url
---
```
...
author: "-"
date: 2019-04-22T14:58:44+08:00     # 原始创建日期，保持不变
lastmod: 2025-10-31T08:30:00+08:00  # 更新为当前东八区时间
url: article-url
categories:
  - 分类
tags:
...
8. ✅ **内容编辑**：完成实际的文章内容修改
9. ✅ **维护记录**：在文章末尾添加或更新 `## 维护记录` 表格（见下文规范）
10. ✅ **格式检查**：确保 Markdown 格式正确

**记忆口诀：编辑文章 = 改文件名 + 改标题 + 检查 URL + 站内互链 + 检查分类 + 更新 lastmod（不改 date）+ 加标签 + 改内容 + 维护记录**

### 维护记录规范

**核心规则：每次 AI 辅助编辑文章后，必须在文章末尾添加或更新 `## 维护记录` 章节。**
